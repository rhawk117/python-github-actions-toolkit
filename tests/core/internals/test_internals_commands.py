...
from action_toolkit.core.internals.interfaces import AnnotationProperties, WorkflowCommand
...
class TestToCommandProperties:
    ...
    def test_partial_properties(self):
        ...
        props = AnnotationProperties(file='test.py', startLine=10)
        result = to_command_properties(props)
    ...
    def test_line_only_properties(self):
        ...
        props = AnnotationProperties(title='Warning', startLine=5, endLine=7)
        result = to_command_properties(props)

        assert result == {'title': 'Warning', 'line': 5, 'endLine': 7}
...
class TestCommand:
    ...
    def test_basic_command(self):
        ...
        cmd = Command(command=WorkflowCommand.DEBUG, properties={}, message='hello')
        assert cmd.as_string() == '::debug::hello'

    def test_command_with_properties(self):
        """Test command with properties"""
        cmd = Command(command=WorkflowCommand.SET_OUTPUT, properties={'name': 'myOutput'}, message='value123')
    ...
    def test_command_with_multiple_properties(self):
        ...
        cmd = Command(
            command=WorkflowCommand.WARNING, properties={'file': 'test.py', 'line': '10'}, message='This is a warning'
        )
    ...
    def test_command_with_none_properties(self):
        ...
        cmd = Command(
            command=WorkflowCommand.ERROR, properties={'key1': 'value1', 'key2': None, 'key3': 'value3'}, message='msg'
        )
        result = cmd.as_string()
    ...
    def test_command_property_escaping(self):
        ...
        cmd = Command(command=WorkflowCommand.SET_ENV, properties={'name': 'KEY:VALUE', 'other': 'a,b'}, message='test')
        result = cmd.as_string()
    ...
    def test_command_message_escaping(self):
        ...
        cmd = Command(command=WorkflowCommand.DEBUG, properties={}, message='Line1\nLine2\rLine3%')
    ...
    def test_command_write(self, mock_stdout):
        ...
        cmd = Command(command=WorkflowCommand.NOTICE, properties={}, message='Test message')
        cmd.write()
    ...
    def test_command_repr(self):
        ...
        cmd = Command(command=WorkflowCommand.ERROR, properties={'file': 'test.py'}, message='Error!')
        repr_str = repr(cmd)
...
class TestIssueCommand:
    ...
    def test_issue_command_basic(self, mock_stdout):
        ...
        issue_command(command=WorkflowCommand.DEBUG, message='Debug message')
    ...
    def test_issue_command_with_properties(self, mock_stdout):
        """Test command with properties"""
        issue_command(command=WorkflowCommand.SET_OUTPUT, properties={'name': 'result'}, message='success')
...
class TestIssueFileCommand:
    ...
    def test_file_command_with_path(self):
        ...
        try:
            result = issue_file_command('OUTPUT', 'test message', file_path=temp_path)
    ...
    def test_file_command_with_env_var(self):
        ...
        try:
            with patch.dict(os.environ, {'TEST_OUTPUT': temp_path}):
                result = issue_file_command('OUTPUT', 'env var message', env_var='TEST_OUTPUT')
    ...
    def test_file_command_empty_env_var(self):
        ...
        with patch.dict(os.environ, {'EMPTY_VAR': ''}):
            with pytest.raises(ValueError, match='Unable to find file path'):
                issue_file_command('OUTPUT', 'message', env_var='EMPTY_VAR')
    ...
    def test_file_command_creates_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = issue_file_command('OUTPUT', 'test message', file_path=file_path)
