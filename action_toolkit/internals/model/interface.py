...
class ModelInterface(metaclass=ModelInterfaceMeta):
    ...
    def _get_alias_fn(self, use_alias_flag: bool) -> Callable[[str], str] | None:
        ...
        if alias_fn is None:
            raise ValueError(use_alias_when_not_defined_err())
        return alias_fn

    @classmethod
    def load(cls, data: dict[str, Any], *, strict: bool = False) -> Self:
    ...
    @classmethod
    def json_loads(cls, json_str: str, *, strict: bool = True) -> Self:
        ...
        *,
        ...
        def _convert(value: Any) -> Any:
            if deep and isinstance(value, ModelInterface):
                return value.dump(deep=True, exclude_none=exclude_none, exclude=exclude, use_alias=use_alias)
    ...
    def json_dumps(
        ...
        **json_kwargs,
        ...
        return json.dumps(
            self.dump(deep=deep, exclude_none=exclude_none, exclude=exclude, use_alias=use_alias),
            indent=indent,
    ...
    def attrs(
        self, *, exclude_none: bool = False, exclude: set[str] | None = None, use_alias: bool = False
    ...
    def map_attrs(self, map_func: Callable[[str, Any], Any]) -> Generator[Any, None, None]:
    ...
    def filter_attrs(self, filter_func: Callable[[str, Any], bool]) -> Generator[tuple[str, Any], None, None]:
    ...
    def as_str(self) -> str:
        ...
        """
        attr_str = ', '.join(f'{f.name}={getattr(self, f.name)!r}' for f in fields(self))
    ...
    def get_field_aliases(self) -> dict[str, str]:
        ...
        return {f.name: alias_fn(f.name) if alias_fn else f.name for f in fields(self)}
